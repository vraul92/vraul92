<div align="center">

# Rahul Vuppalapati 🧠

**Senior Data Scientist** | **Applied AI Researcher** | **GenAI & Computer Vision**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/vrc7)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/vraul92)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:vrc7.ds@gmail.com)

📍 Bengaluru, India | 8+ Years | Previously: **Apple** | **Walmart** | **IBM**

</div>

---

## 🧬 Research Focus

I'm a research-oriented Data Scientist building **zero-shot**, **multi-modal AI systems** that understand and interact with the world through natural language and visual perception.

```
┌─────────────────────────────────────────────────────────────┐
│  🔬 CURRENT RESEARCH                                        │
├─────────────────────────────────────────────────────────────┤
│  • Zero-Shot Medical Image Segmentation (SAM + CLIP)       │
│  • Multi-Modal RAG for Documents with Visual Understanding │
│  • Neural Radiance Fields for Real-Time SLAM               │
│  • LLM-Powered Time Series Anomaly Detection               │
│  • Foundation Models for Healthcare AI                     │
└─────────────────────────────────────────────────────────────┘
```

---

## 🚀 Featured Projects

<div align="center">

### 🏥 Medical SAM-CLIP
**Zero-Shot Medical Image Segmentation via Natural Language**

[![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)](https://python.org)
[![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)](https://pytorch.org)
[![OpenAI](https://img.shields.io/badge/CLIP-412991?style=flat&logo=openai&logoColor=white)](https://openai.com/research/clip)
[![Meta](https://img.shields.io/badge/SAM-0668E1?style=flat&logo=meta&logoColor=white)](https://segment-anything.com)

> *"segment the left lung"* → Instant segmentation. No training required.

**[🔗 View Project](https://github.com/vraul92/medical-sam-clip)**

**Key Features:**
- ✅ Zero-shot medical image segmentation (X-ray, CT, MRI, pathology)
- ✅ SAM + CLIP fusion with attention visualization
- ✅ Interactive Gradio web interface
- ✅ Reproducible benchmarks (IoU: 0.847, Dice: 0.917)
- ✅ Interactive Plotly dashboards

```bash
pip install -r requirements.txt && python app.py
# Opens at http://localhost:7860 🌐
```

</div>

---

## 🛠️ Tech Stack

### Languages & Frameworks
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![JAX](https://img.shields.io/badge/JAX-4285F4?style=flat&logo=google&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)

### GenAI & LLMs
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=chainlink&logoColor=white)
![LlamaIndex](https://img.shields.io/badge/LlamaIndex-F9A825?style=flat&logo=llama&logoColor=black)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)
![Anthropic](https://img.shields.io/badge/Anthropic-D4A574?style=flat&logo=anthropic&logoColor=white)

### Computer Vision
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white)
![Pillow](https://img.shields.io/badge/Pillow-3C3C3C?style=flat&logo=python&logoColor=white)
![Albumentations](https://img.shields.io/badge/Albumentations-FF6B6B?style=flat&logo=image&logoColor=white)

### Cloud & MLOps
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoft-azure&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=flat&logo=google-cloud&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)

---

## 📊 GitHub Stats

<div align="center">

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=vraul92&show_icons=true&theme=dracula&hide_border=true&count_private=true)](https://github.com/vraul92)

[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=vraul92&layout=compact&theme=dracula&hide_border=true)](https://github.com/vraul92)

[![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=vraul92&theme=dracula&hide_border=true)](https://github.com/vraul92)

</div>

---

## 🏆 Career Highlights

| Company | Role | Impact |
|---------|------|--------|
| **Apple** | Senior Data Scientist | GenAI systems for Customer Support |
| **Walmart** | Principal Data Scientist | $50M+ revenue optimization via ML |
| **IBM** | Data Scientist | Enterprise AI solutions for Healthcare|

### Key Achievements
- 🎯 **$50M+** business impact across roles
- 🧬 **8+ years** in ML/CV/NLP/GenAI
- 📄 **Zero-shot learning** research focus
- 🏥 **Healthcare AI** specialization
- 🚀 **Production deployment** at scale

---

## 📚 Latest Research & Writing

<!-- BLOG-POST-LIST:START -->
- [Multi-Modal RAG: Teaching LLMs to See](https://github.com/vraul92) - Coming soon
- [Zero-Shot Medical Segmentation with SAM + CLIP](https://github.com/vraul92/medical-sam-clip) - 2024
- [Neural SLAM with Real-Time NeRF](https://github.com/vraul92) - In progress
<!-- BLOG-POST-LIST:END -->

---

## 💡 Open Source Contributions

- 🔬 Contributing to [HuggingFace Transformers](https://github.com/huggingface/transformers)
- 🏥 Medical AI tools for low-resource settings
- 📊 Reproducible ML experiment templates

---

## 🌱 Currently Learning

- **Mamba State Space Models** - Efficient sequence modeling
- **Mixture of Experts (MoE)** - Scaling LLMs efficiently  
- **Neural Radiance Fields** - 3D scene understanding
- **Diffusion Models** - Generative AI for healthcare

---

## 📫 Let's Connect

<div align="center">

I'm always interested in:
- 🤝 Research collaborations in GenAI/CV/Healthcare
- 💼 Senior IC Data Scientist / Applied Research roles
- 🧪 Open source projects with real-world impact

**📧 vrc7.ds@gmail.com** | **[LinkedIn](https://linkedin.com/in/vrc7)**

</div>

---

<div align="center">

⭐ **If you find my work interesting, let's chat!** ⭐

![Visitor Badge](https://visitor-badge.laobi.icu/badge?page_id=vraul92.vraul92)

</div>

---

